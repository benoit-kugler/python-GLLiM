...
def e1(R, x):
    ...
    y = np.where(mask, np.exp(-2 / arg[mask]), 0)
    return y
...
def import_geometries(context):
    ...
    def F(X):  # vector
        Xfull = np.concatenate((geom[:, 0].T, numpy.repeat(X[None, :], D, axis=0)), axis=1)
        return Hapke_vect(*Xfull.T)

    return F


h = HapkeContext(None)
F = import_geometries(h)

X0 = h.get_X_sampling(1)

# print(F(X[0]) - h.F(X)[0])

dFonepoint = jacobian(F)


def dF(X):
    return np.array([dFonepoint(x) for x in X])


### --------------- OK but slow ------------------ ###

print(timeit("dFonepoint(X0[0])", number=5, globals=globals()))
